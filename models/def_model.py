...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        encoder_layer = DeformableTransformerEncoderLayer(TN_HIDDEN_DIM, TN_DIM_FEEDFORWARD, dropout=TN_DROPOUT , n_heads=TN_NHEADS)
        ...
        self.encoder = DeformableTransformerEncoder(encoder_layer, TN_ENC_LAYERS)
        # decoder_layer = TransformerDecoderLayer(TN_HIDDEN_DIM, TN_NHEADS, TN_DIM_FEEDFORWARD, TN_DROPOUT, "relu", True)
        decoder_layer = DeformableTransformerDecoderLayer(TN_HIDDEN_DIM, TN_DIM_FEEDFORWARD, dropout=TN_DROPOUT , n_heads=TN_NHEADS)
        ...
        self.decoder = DeformableTransformerDecoder(decoder_layer, TN_DEC_LAYERS)
        ...
        self.query_embed = nn.Embedding(VOCAB_SIZE, TN_HIDDEN_DIM*2)
        ...
        self.reference_points = nn.Linear(TN_HIDDEN_DIM, 2)
        if self.num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, TN_HIDDEN_DIM, kernel_size=1),
                    nn.GroupNorm(32, TN_HIDDEN_DIM),
                ))
            for _ in range(self.num_feature_levels - num_backbone_outs):
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, TN_HIDDEN_DIM, kernel_size=3, stride=2, padding=1),
                    nn.GroupNorm(32, TN_HIDDEN_DIM),
                ))
                in_channels = TN_HIDDEN_DIM
            self.input_proj = nn.ModuleList(input_proj_list)
        else:
            self.input_proj = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(self.backbone.num_channels[0], TN_HIDDEN_DIM, kernel_size=1),
                    nn.GroupNorm(32, TN_HIDDEN_DIM),
                )])
    ...
    def forward_enc(self, srcs, masks, query_embed=None):
        ...
        src_flatten = torch.cat(src_flatten, 1).to(DEVICE)
        #src_flatten.shape = 50 129 512
        #mask_flatten = torch.cat(mask_flatten, 1).to(DEVICE)
        #lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1).to(DEVICE)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=src_flatten.device).to(DEVICE)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1])).to(DEVICE)
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1).to(DEVICE)
    ...
    def Eval(self, ST, QRS):
        ...
        B, N, R, C = ST.shape
        if not isinstance(ST, NestedTensor):
            samples = nested_tensor_from_tensor_list(ST)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                masks.append(mask)
                pos.append(pos_l)
        memory, spatial_shapes, level_start_index, valid_ratios = self.forward_enc(srcs, masks, query_embed=None)
        bs, _, c = memory.shape
        OUT_IMGS = []
        for i in range(QRS.shape[1]):          
            QR = QRS[:, i, :]
            QR_EMB = self.query_embed.weight[QR]
            QR_EMB, tgt = torch.split(QR_EMB, c, dim=2)
            QR_EMB = QR_EMB.to(DEVICE)
            tgt = tgt.to(DEVICE)
            reference_points = self.reference_points(QR_EMB).sigmoid().to(DEVICE)
            #init_reference_out = reference_points.to(DEVICE)

            # decoder
            hs, inter_references = self.decoder(tgt, reference_points, memory,
                                                spatial_shapes, level_start_index, valid_ratios, QR_EMB)
            h = torch.cat([QR_EMB, hs], -1)
            h = self.linear_q(h)
            h = h.contiguous() 
...
class TRGAN(nn.Module):
    ...
    def __init__(self):
        ...
        with open('/mydrive/MyDrive/deformable/deformbale_hwt/english_words.txt', 'rb') as f:
        ...
        f = open('/mydrive/MyDrive/deformbale/deformable_hwt/mytext.txt', 'r') 
